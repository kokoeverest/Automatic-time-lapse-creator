...
def test_source_initializes_correctly_with_default_config():
    ...
    with (
        ...
        assert not sample_source.is_valid_stream
...
def test_get_frame_bytes_calls_correct_method_for_static_webcam_url(
    ...
    with patch.object(
        sample_source, "fetch_image_from_static_web_cam", return_value=expected_result
    ) as mock_source:
        # Act
...
def test_get_frame_bytes_calls_correct_method_for_stream_url(
    # sample_source: Source,
):
    # Arrange
    source = Source(
    ...
    with (
        ...
        # Act
        actual_result = source.get_frame_bytes()
